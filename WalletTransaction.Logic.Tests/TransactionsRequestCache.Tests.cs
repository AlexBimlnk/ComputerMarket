...
public class TransactionsRequestCacheTests
{
    ...
    [Fact(DisplayName = $"The {nameof(TransactionsRequestCache)} can get {nameof(CancellationToken)} by key.")]
    [Trait("Category", "Unit")]
    public void CanGetCancelattionTokenByKey()
    {
        // Arrange
        var requests = new TransactionRequest[]
        {
        ...
        // Act
        var exception = Record.Exception(() => 
            _ = transactionsRequestCache.GetCancellationTokenByRequest(new InternalID(5)));

        // Assert
        exception.Should().BeNull();
    }

    [Fact(DisplayName = $"The {nameof(TransactionsRequestCache)} can't get {nameof(CancellationToken)} by key if request not exist.")]
    [Trait("Category", "Unit")]
    public void CanNotGetCancelattionTokenByKeyIfNotExists()
    {
        // Arrange
        var requests = new TransactionRequest[]
        {
        ...
        // Act
        var exception = Record.Exception(() =>
            _ = transactionsRequestCache.GetCancellationTokenByRequest(new InternalID(6)));
    ...
    public void CanCancelRequest()
    {
        ...
        var token = transactionsRequestCache.GetCancellationTokenByRequest(new InternalID(5));
