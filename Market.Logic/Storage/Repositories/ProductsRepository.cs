...
public sealed class ProductsRepository : IProductsRepository
{
    ...
    /// <inheritdoc/>
    public async Task AddOrUpdateProductAsync(Product entity, CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(entity);
    ...
    /// <inheritdoc/>
    public async Task<bool> ContainsProductAsync((ID, ID) key, CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();

        return await _context.Products.AsNoTrackingWithIdentityResolution().ContainsAsync(new TProduct() { ItemId = key.Item1.Value , ProviderId = key.Item2.Value }, token).ConfigureAwait(false);
    }
    ...
    /// <inheritdoc/>
    public void RemoveProduct((ID, ID) key) => _context.Products.Remove(new () { ProviderId = key.Item2.Value, ItemId = key.Item1.Value});
    ...
    public IEnumerable<Product> GetAllItemProducts(Item item) =>
        _context.Products
        .Where(x => x.ItemId == item.Key.Value)
        .Select(x => ConvertFromStorageModel(x))
        .AsEnumerable();


    /// <inheritdoc/>
    public async Task AddProductRangeAsync(IReadOnlyCollection<Product> products)
    {
        ArgumentNullException.ThrowIfNull(products);

        await Task.WhenAll(products.Select(async x => await AddOrUpdateProductAsync(x)));
    }
